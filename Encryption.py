...
def main():
    ...
    while True:
        try:
            userInput  = sys.stdin.readline() # waits for the next command from the user
            if not userInput: #break if nothing
                break
            userInput = userInput.strip() #removes unnecessary space
            if not userInput:
                continue

        # parse the userInput from the user into the first half as the instruction and the other as a part
            parts = userInput.split(" ", 1)   # split at the first space only, could be a cipher otherwise
        ...
        # If something unexpected happens, write to stderr so driver/stdout isn't confused
